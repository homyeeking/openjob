...
4. If registration is requested:
   - Run `node cli/bin/openjob init` if `~/.agents/jobs.json` does not exist.
   - Run `node cli/bin/openjob add ~/.agents/jobs/<job-name>`.
   - Run `node cli/bin/openjob list` to confirm registration.
